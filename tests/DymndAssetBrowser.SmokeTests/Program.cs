...
try
{
    ...
    File.WriteAllBytes(Path.Combine(faRoot, "photo.JPG"), []); File.WriteAllBytes(Path.Combine(faRoot, "ignored.webm"), []);
...
if (stateRootArg >= 0 && stateRootArg + 1 < args.Length)
{
    ...
    await liveViewModel.ShowBuildAsync();
    ...
    Expect(liveViewModel.BuildFloorSampleAssets.Count is > 0 and <= 240
        ...
        && liveViewModel.BuildRightPaletteRows.Any(row => row.IsHeader && row.LeftSection == BuildPaletteSection.FloorSamples)
        && liveViewModel.BuildRightPaletteRows.Any(row => row.IsHeader && row.LeftSection == BuildPaletteSection.TrimSamples),
    ...
    Expect(previewHeaders.Count == 2
        ...
        && liveViewModel.BuildRightPaletteRows.First().LeftSection == BuildPaletteSection.FloorSamples
        && previewHeaders[1].LeftSection == BuildPaletteSection.Detailing
        && liveViewModel.BuildRightPaletteRows.Where(row => row.IsHeader).Last().LeftSection == BuildPaletteSection.TrimSamples,
        "Planner preview did not retain independent Walls/Detailing and Floor/Trim columns.");
    ...
    Expect(wallFloorHeader.LeftHeaderHasBody == (liveViewModel.BuildWallAssets.Count > 0)
        && liveViewModel.BuildRightPaletteRows.First().LeftHeaderHasBody == (liveViewModel.BuildFloorAssets.Count > 0),
...
Console.WriteLine("All DYM&D Asset Browser taxonomy, source, filter, index, and cache smoke tests passed.");
return 0;
